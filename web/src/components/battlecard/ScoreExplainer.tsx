...
export function ScoreExplainer({ score }: { score: ScoreBreakdown }) {
  return (
    <section className="rounded-card border border-neutral-200 p-4 dark:border-neutral-700">
      <h3 className="mb-3 text-sm font-semibold">Why this area scored {score.total.toFixed(2)}</h3>
      <ul className="space-y-2">
        {score.contributions.map((c) => (
          <li key={c.signal}>
            <div className="flex items-center justify-between text-xs">
              <span className="font-medium">{SIGNAL_LABELS[c.signal] ?? c.signal}</span>
              <span className="text-neutral-500">
                {c.contribution.toFixed(2)} ({(c.weight * 100).toFixed(0)}% weight)
              </span>
            </div>
            <div className="mt-1 h-1.5 w-full rounded-full bg-neutral-200 dark:bg-neutral-800">
              <div
                className="h-1.5 rounded-full bg-light-accent dark:bg-dark-accent"
